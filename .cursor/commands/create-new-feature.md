...
Please run `/plan-feature {version} {feature-name}` first to create the spec.
```
...
**How to explore:**
- **Discovery**: Use glob for filename searches, grep for content searches
- **Analysis**: Read entry points, trace data flow, map function calls
...
- Look in common locations (`src/`, `tests/`, `examples/`)

**Output format:**
- File locations with `file:line` references
...
Create tests in **verification mode** (testing existing implementation):
...
**Test structure:**
- Match naming patterns from existing tests (`test_*.py`)
- Use descriptive names: `test_<function>_<scenario>_<expected_result>`
- One behavior per test with Arrange-Act-Assert pattern
...
**Best practices:**
- Mock external dependencies (I/O, APIs, time)
...
Review the implementation against quality criteria:

**Design Principles (Ousterhout's):**
- Is the interface simple relative to functionality? (Deep Modules)
- Is complexity hidden inside, not pushed to callers? (Simple Interface)
- Would a new reader understand this quickly? (Design for Reading)
- Do comments explain *why*, not *what*? (Non-Obvious Comments)

**Red Flags to Check:**

| Red Flag | Question to Ask |
|----------|-----------------|
| Shallow Module | Is interface as complex as implementation? |
| Information Leakage | Does same decision appear in multiple places? |
| Pass-Through Method | Does this just forward to another method? |
| Repetition | Have I seen this pattern elsewhere? |
| Vague Name | Would a new reader know what this is? |
| Non-Obvious Code | Can I understand without tracing execution? |

**Bug Patterns to Check:**
- Logic: Edge cases, off-by-one, null references
- Error Handling: Missing catches, bare excepts, swallowed errors
- Type Safety: Annotations present and correct, None handling
- Resource Management: File handles closed, context managers used

**mlforge-Specific Checks:**
- Imports follow `import mlforge.X as X` convention
- Google-style docstrings on public functions
- Error classes follow project patterns (cause, hint)
- Type annotations use modern syntax (`str | None` not `Optional[str]`)
