...
6. **CLI `init` Command** - Initialize new mlforge projects with boilerplate structure
7. **CLI Enhancements** - New subcommands for listing and inspecting entities, sources, and versions
8. **Detailed Examples** - Comprehensive examples for complex ML pipelines
...
```bash
mlforge init my-project --online redis    # Include RedisStore config
mlforge init my-project --engine duckdb   # Use DuckDB as default engine
mlforge init my-project --store s3        # Configure S3Store
```
...
Comprehensive examples demonstrating complex ML pipelines.

**New Examples:**

```
examples/
├── transactions/           # Existing - basic usage
├── transactions-online/    # Existing - Redis online store
├── fraud-detection/        # NEW - Full fraud detection pipeline
│   ├── src/
│   │   └── fraud/
│   │       ├── definitions.py
│   │       ├── features.py
...
│   │   └── exploration.ipynb
│   └── README.md
└── recommendation/         # NEW - User-item recommendations
    ├── src/
    │   └── reco/
    │       ├── definitions.py
    │       ├── user_features.py
...
**Fraud Detection Example Covers:**
- Multiple entity types (user, merchant, card)
- Rolling aggregations at different granularities
- Point-in-time correct training data
- Model training with scikit-learn
- Online serving with Redis

**Recommendation Example Covers:**
...
- **v0.7.0**: DynamoDB online store
- **v0.7.0**: FeatureSpec for column-level selection
- **v0.7.0**: MLflow integration
- **v0.8.0**: PySpark engine
- **v0.8.0**: Databricks (Unity Catalog, Online Tables)
- **v0.8.0**: Apache Iceberg support
- **v0.9.0**: Prometheus/Grafana metrics
- **v0.9.0**: Text embedding support
